...
if(strlen($_SESSION['login']) == 0) {   
    ...
    if(isset($_POST['update'])) {    
        ...
        $fnameValid = preg_match("/^[a-zA-Z\s]*$/", $fname);
        $mobileValid = preg_match("/^\d{10,11}$/", $mobileno);

        if(!$fnameValid) {
            $fnameError = "Invalid characters in Full Name. Please remove special characters.";
        }

        if(!$mobileValid) {
            $mobileError = "Mobile Number should be 10 to 11 digits long.";
        }

        if($fnameValid && $mobileValid) {
