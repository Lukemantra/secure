...
if(strlen($_SESSION['alogin'])==0) {   
    ...
    if(isset($_POST['update'])) {
        ...
        // Validation: Check if the author's name is not empty
        if(empty($author)) {
        ...
        // Validation: Check if the author's name contains only alphabetic characters
        if(!preg_match("/^[a-zA-Z]+$/", $author)) {
            $_SESSION['error'] = "Author name can only contain alphabetic characters.";
            header('location:manage-authors.php');
            exit; // Stop further execution
        }
